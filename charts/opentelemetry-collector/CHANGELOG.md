...
- [Feat] `presets.pprofReceiver.pull.defaultProfileTypes`: each entry accepts an optional `interval` that overrides `collectionInterval` for that profile type alone.
- [Feat] `presets.pprofReceiver.pull.defaultProfileTypes`: each entry accepts an optional `sampleTypes` list, keeping only the named pprof sample types from that scrape. Every sample type in a scraped profile becomes its own OTLP profile carrying a full copy of every stack, so an endpoint that reports four sample types is stored four times over. `heap` and `allocs` are the same Go profile and both report all four memory sample types, but each is authoritative for only one pair: `heap` reports `inuse_*` as a live gauge while its `alloc_*` are running totals since process start, and `allocs` reports `alloc_*` as a delta over `seconds` while its `inuse_*` are deltas of a gauge and can go negative. The defaults now keep `inuse_objects`/`inuse_space` from `heap` and `alloc_objects`/`alloc_space` from `allocs`, halving stored memory profile volume and dropping only duplicated or unusable values. Entries without `sampleTypes` keep everything the endpoint returns, and the preset renders no filter at all when no entry sets it.
- [Feat] `presets.pprofReceiver.pull`: the default `heap` scrape now uses `interval: 300s` instead of the shared 60s. `heap` and `allocs` are the same Go profile and carry the same four sample types (`alloc_objects`, `alloc_space`, `inuse_objects`, `inuse_space`), so scraping both at 60s sent the memory profile twice per interval. They differ in window: `allocs` is requested with `seconds`, making it a delta bounded by its sampling window, while `heap` is served as the profile accumulated since process start, repeating every allocation site the process has touched rather than only those active in the last window, with `alloc_*` reported as running totals rather than a rate. On a small Go service a `heap` scrape carried about twice the samples and 2.3x the bytes of the matching `allocs` delta. Since the stored volume behind a flamegraph is samples per scrape times scrapes times instances times the length of the time range being viewed, the larger and more redundant of the two streams is now scraped five times less often. `heap` remains the only source of the live-heap (`inuse`) sample types, and those are a gauge, so a longer interval costs resolution rather than data. Restore the previous behaviour by setting `interval: 60s` on the `heap` entry.
